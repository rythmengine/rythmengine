 ...
 * the properties or a string of the class name</li>
 * 
 * </ul>
 */
public enum RythmConfigurationKey {
    ...
    CACHE_SERVICE_IMPL("cache.service.impl") {
        @Override
        protected Object getDefVal(Properties properties) {
            Boolean cacheEnabled = CACHE_ENABLED.getConfiguration(properties);
    ...
    DEFAULT_CACHE_TTL("default.cache_ttl") {
        @Override
        public <T> T getConfiguration(Properties properties) {
            String k = getKey();
    ...
    HOME_TEMPLATE("home.template") {
        @Override
        public <T> T getConfiguration(Properties properties) {
            ITemplateResourceLoader loader = RESOURCE_LOADER_IMPL.getConfiguration(properties);
            if (null != loader) {
                return null;
            }
            try {
                File f = getFile(getKey(), properties);
                if (null == f) {
    ...
    HOME_TMP("home.tmp"){
        @Override
        public <T> T getConfiguration(Properties properties) {
            Rythm.Mode mode = ENGINE_MODE.getConfiguration(properties);
            if (mode.isProd()) {
                return null;
            }
            File f = getFile(getKey(), properties);
    ...
    private static Boolean getAsBoolean(String key, Properties properties) {
    ...
    private Boolean getEnabled(String key, Properties properties) {
        String k0 = key.replace(".enabled", "");
        String k1 = k0 + ".disabled";
        if (properties.contains(k1)) {  
            return (!getAsBoolean(k1, properties));
        }
        if (properties.contains(key)) {
            return getAsBoolean(key, properties);
        }
        if (properties.contains(k0)) {
            return getAsBoolean(k0, properties);
        }
        Object defVal = getDefVal(properties);
        return (null == defVal) ? false : (Boolean)defVal;
    }
    
    private <T> T getImpl(String key, Properties properties) {
        Object v = properties.get(key);
        if (null == v) {
            String k0 = key.replace(".impl", "");
            v = properties.get(key);
            if (null == v) {
                v = getDefVal(properties);
            }
        }
        if (null == v) {
            return null;
    ...
    private static File getFile(String key, Properties properties) {
        Object v = properties.get(key);
        if (null == v) {
            return null;
        }
    ...
    public <T> T getConfiguration(Properties properties) {
        ...
        if (key.endsWith(".enabled")) {
            return (T)getEnabled(key, properties);
        }
        if (key.endsWith(".impl")) {
            return getImpl(key, properties);
        }
        Object o = properties.get(key);
        if (null == o) {
            return (T) getDefVal(properties);
        } else {
            return (T) o;
        }
    }
