...
public class BytecodeCache {
    private static final ILogger logger = Logger.get(BytecodeCache.class);

    private RythmEngine engine() {
        return classLoader.engine;
    }

    private TemplateClassLoader classLoader = null;
    BytecodeCache (TemplateClassLoader classLoader) {
        ...
        try {
            ...
            if (f.exists()) {
                ...
                if (!hash(source).equals(hash.toString())) {
                    if (logger.isTraceEnabled()) logger.trace("Bytecode too old (%s != %s)", hash, hash(source));
                    return null;
                }
    ...
    public void cacheBytecode(byte[] byteCode, String name, String source) {
        try {
            ...
            if(!name.contains("/") && !name.contains("{")) {
                f = new File(engine().tmpDir, "classes/"+(name.replace(".", "/"))+".class");
     ...
     */
    File cacheFile(String id) {
        File dir = new File(engine().tmpDir, "bytecode/");
        if (!dir.exists() && engine().tmpDir != null) {
            dir.mkdirs();
        }
