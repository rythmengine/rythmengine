...
//    }
    ...
    public CompiledTemplate compile(String template, String className, boolean requireCompiled) {
        if (null == className) className = uniqueClassName();
    ...
    @SuppressWarnings("unchecked")
    private static Class<? extends ITemplate> compile_(String srcCode, String className) {
        ...
        JavaCompiler.CompilationTask task = compiler.getTask(out, fileManager, dianosticListener, options, classes, compilationUnits);
